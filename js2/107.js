...
W.shStarAddGroup = async function(){
...
W.shStarDelGroup = function(gid){
  confirm2('ยืนยันการลบกลุ่ม','ต้องการลบกลุ่มนี้ใช่หรือไม่?', async ()=>{
    const cid=getCid(); if(!cid || !W.__currentEditSetId) return;
    const cd=starCourseData(cid);
    const currentSet = cd.sets.find(s => s.id === W.__currentEditSetId);
    if(!currentSet) return;
...
W.shStarRenameGroup = async function(gid){
  const cid=getCid(); if(!cid || !W.__currentEditSetId) return;
  const cd=starCourseData(cid);
  const currentSet = cd.sets.find(s => s.id === W.__currentEditSetId);
  if(!currentSet) return;
...
W.shStarAddMember = async function(gid, mid){
  const cid=getCid(); if(!cid || !W.__currentEditSetId) return;
  const cd=starCourseData(cid);
  const currentSet = cd.sets.find(s => s.id === W.__currentEditSetId);
  if(!currentSet) return;
...
W.shStarRemoveMember = async function(gid, mid){
  const cid=getCid(); if(!cid || !W.__currentEditSetId) return;
  const cd=starCourseData(cid);
  const currentSet = cd.sets.find(s => s.id === W.__currentEditSetId);
  if(!currentSet) return;
