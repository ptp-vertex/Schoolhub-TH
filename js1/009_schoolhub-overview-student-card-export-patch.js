...
(function(){
  ...
  window.renderCourseOverview = function(){
    ...
    plans.forEach(function(p){ var isChecklist=Number(p.maxScore)===0; var subtitle=isChecklist?'เช็คงาน':'เต็ม '+window.formatScoreDisplay(p.maxScore,2); thead += '<th class="text-center bg-indigo-50 text-indigo-700 summary-score-col" title="คลิกเพื่อดูรายละเอียด: สัปดาห์ '+esc(p.week)+' | '+esc(p.title)+' | '+esc(subtitle)+'"><div class="inline-flex items-center gap-0.5"><button type="button" onclick="showPlanDetail(\''+cid+'\', \''+p.id+'\')" class="week-detail-btn inline-flex items-center justify-center bg-white border border-indigo-200 text-primary font-bold hover:bg-primary hover:text-white transition shadow-sm">'+esc(p.week)+'</button><button type="button" onclick="jumpToScoreEntry(\''+cid+'\', \''+p.week+'\', event)" title="ไปหน้ากรอกคะแนนสัปดาห์นี้" class="week-goto-score-btn inline-flex items-center justify-center bg-white border border-emerald-200 text-emerald-600 hover:bg-emerald-500 hover:text-white transition shadow-sm" style="width:18px;height:18px;border-radius:6px;font-size:9px;line-height:1"><i class="fas fa-pen"></i></button></div></th>'; if(!isChecklist) totalMax = window.addScoreToTotal(totalMax, p.maxScore, 2); });
